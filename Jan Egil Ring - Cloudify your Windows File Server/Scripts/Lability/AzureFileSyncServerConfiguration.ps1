Configuration AzureFileSyncServerConfiguration {
<#
...
W        xDnsServer (v1.5.0 or later):            https://github.com/PowerShell/xDnsServer
#>
    param (
    ...
    node $AllNodes.Where({$_.Role -in 'DC'}).NodeName {
        ...
        {
            Name = "Prior to domain controller promotion"
        }
     ...
     node $AllNodes.Where({$_.Role -in 'CLIENT','FileServer','ClusteredFileServer'}).NodeName {
        ## Flip credential into username@domain.com
    ...
    node $AllNodes.Where({$_.Role -in 'FileServer'}).NodeName {
        ## Flip credential into username@domain.com
        $upn = '{0}@{1}' -f $Credential.UserName, $node.DomainName;
        $domainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ($upn, $Credential.Password);
    ...
    } #end nodes FileServer

    node $AllNodes.Where({$_.Role -in 'ClusteredFileServer'}).NodeName {
        ## Flip credential into username@domain.com
        $upn = '{0}@{1}' -f $Credential.UserName, $node.DomainName;
        $domainCredential = New-Object -TypeName System.Management.Automation.PSCredential -ArgumentList ($upn, $Credential.Password);
